...
/**
 * Factory class for creating named color palettes.
 * Provides access to predefined palettes like viridis, hot, cool, etc.
 */
public final class NamedPalettes {
    ...
    /**
     * Creates the viridis palette.
     * Viridis is a perceptually uniform, colorblind-friendly colormap.
     * <p>
     * Derived from "viridis - Colorblind-Friendly Color Maps for R"
     * Garnier et al (2021)
     * https://CRAN.R-project.org/package=viridis
     *
     * @return Viridis color palette
     */
    public static ColorPalette viridis() {
        ColorPalette.Builder builder = ColorPalette.builder();

        // Use sampled gradient points from the 256-color viridis map
        for (int i = 0; i < VIRIDIS_COLORS.length; i++) {
            double position = i / (double) (VIRIDIS_COLORS.length - 1);
            Color color = Color.fromRGB24(VIRIDIS_COLORS[i]);
            builder.addGradientPoint(position, color);
        }
    ...
    /**
     * Viridis colormap - 256 RGB hex values
     */
    ...
    /**
     * Creates a "hot" palette (black → red → yellow → white).
     */
    public static ColorPalette hot() {
        return ColorPalette.builder()
                .addGradientPoint(0.0, Color.BLACK)
                .addGradientPoint(0.33, Color.RED)
                .addGradientPoint(0.67, Color.YELLOW)
                .addGradientPoint(1.0, Color.WHITE)
                .build();
    }

    /**
     * Creates a "cool" palette (cyan → magenta).
     */
    public static ColorPalette cool() {
        return ColorPalette.builder()
                .addGradientPoint(0.0, Color.CYAN)
                .addGradientPoint(1.0, Color.MAGENTA)
                .build();
    }

    /**
     * Creates a "rainbow" palette (red → yellow → green → cyan → blue → magenta).
     */
    public static ColorPalette rainbow() {
        return ColorPalette.builder()
                .addGradientPoint(0.0, Color.RED)
                .addGradientPoint(0.2, Color.YELLOW)
                .addGradientPoint(0.4, Color.GREEN)
                .addGradientPoint(0.6, Color.CYAN)
                .addGradientPoint(0.8, Color.BLUE)
                .addGradientPoint(1.0, Color.MAGENTA)
                .build();
    }

    /**
     * Creates the default cubehelix palette.
     */
    public static ColorPalette cubehelix() {
        return ColorPalette.builder()
